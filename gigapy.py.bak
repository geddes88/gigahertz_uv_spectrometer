...
#        self.giga=gigahertz()
    ...
    def threaded_solar(self):
        ...
        #print self.schedule
        f.clf()
        f.subplots_adjust(right=0.63,top=0.95,hspace=0.4)
        ...
        lines1,=self.time_series.plot([],[],'ow')
        lines2,=self.time_series.plot([],[],'ob')
        lines3,=self.time_series.plot([],[],'og')
        lines4,=self.time_series.plot([],[],'oy')
        lines5,=self.time_series.plot([],[],'or')
        lines6,=self.time_series.plot([],[],'om')
        lines7,=self.time_series.plot([],[],'oc')
        ...
        self.time_series.plot(self.schedule,self.tasks_out,'ow')
       # self.time_series.xaxis.set_tick_params(rotation=45)
        self.time_series.legend(lines,labels,loc='center right',bbox_to_anchor=(1.7, 0.5))
...
#            
        if len(all_scans)!=0:
            ...
            self.time_series.plot(start,sza_in,'mo',label='Successful High Density')
            create_panda_file(def_paths_files['datapath'],all_data,start,sza_in,hd=True)

        else:
            self.write_output("Measurment Failed")
            self.time_series.plot(start,sza_in,'co',label='Failed High Density')
        self.plot_window.show()
...
#            
        if len(all_scans)!=0:
            self.write_output("Measurment Complete, "+str(fail_counts)+" Failed Scans")
            if mode =="manual":
                self.time_series.plot(start,sza_in,'bo',label='Successful Manual')
            else:
                self.time_series.plot(start,sza_in,'go',label='Successful Scheduled')
            create_panda_file(def_paths_files['datapath'],all_data,start,sza_in)

        else:
            self.write_output("Measurment Failed")
            if mode =="manual":
                self.time_series.plot(start,sza_in,'yo',label='Failed Manual')
            else:
                self.time_series.plot(start,sza_in,'ro',label='Failed Scheduled')
        self.plot_window.show()
